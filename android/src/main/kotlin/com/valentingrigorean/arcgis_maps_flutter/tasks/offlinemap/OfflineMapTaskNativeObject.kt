...
import com.valentingrigorean.arcgis_maps_flutter.convert.tasks.offlinemap.toGenerateOfflineMapParametersOrNull
...
class OfflineMapTaskNativeObject(objectId: String, task: OfflineMapTask) :
    ...
    private fun generateOfflineMap(data: Map<*, *>, result: MethodChannel.Result) {
        val parameters = data["parameters"]?.toGenerateOfflineMapParametersOrNull()!!
        val downloadDirectory = data["downloadDirectory"] as String
        val offlineMapJob = nativeObject.createGenerateOfflineMapJob(parameters, downloadDirectory)
